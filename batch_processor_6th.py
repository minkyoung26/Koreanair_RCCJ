...
def load_file_and_clean_columns(file_path):
    ...
    print(f"  └ '{os.path.basename(file_path)}' 파싱 및 Country Name 추출 중...")
    ...
    # 📌 요청 반영: Country Name Code 대신 Country Name 사용
    c_orig_country_name = get_col("Trip Origin Country Name") or get_col("Trip Origin Country")
    c_dest_country_name = get_col("Trip Destination Country Name") or get_col("Trip Destination Country")
    ...
    clean_pdf["Trip Origin Country Name"] = pdf[c_orig_country_name] if c_orig_country_name else ""
    clean_pdf["Trip Destination Country Name"] = pdf[c_dest_country_name] if c_dest_country_name else ""
...
def process_and_aggregate_6th_data(cy_file_path, py_file_path, dim_file_path, output_parquet_path="cache_6th_data.parquet"):
    print("🚀 6수송 RAW 데이터 정밀 재가공 및 사전 집계를 시작합니다...")
    ...
    # 📌 Trip O&D v.v. (일본 공항 무조건 앞으로 정렬)
    def make_vv_market(mkt_str, o_code, d_code, o_cntry_name):
        if not mkt_str or "/" in str(mkt_str): return str(mkt_str)
        p = str(mkt_str).replace(" ", "").split("-")
        if len(p) == 2:
            a1, a2 = p[0], p[1]
            is_o_jp = str(o_cntry_name).upper().strip() in ["JAPAN", "JP", "JPN"]
            if is_o_jp: return f"{a1}-{a2} v.v."
            else: return f"{a2}-{a1} v.v."
        return f"{mkt_str} v.v."

    is_jp_expr = pl.col("Trip Origin Country Name").cast(pl.Utf8).str.to_uppercase().str.strip_chars().is_in(["JAPAN", "JP", "JPN"])
    ...
    df_processed = df_merged.with_columns([
        ...
        pl.struct(["Trip Market", "Trip Origin Code", "Trip Destination Code", "Trip Origin Country Name"]).map_elements(
            lambda x: make_vv_market(x["Trip Market"], x["Trip Origin Code"], x["Trip Destination Code"], x["Trip Origin Country Name"]), return_dtype=pl.Utf8
        ...
        # 📌 OD Region용 매핑 키: Origin이 Japan이면 Destination Country Name, 아니면 Origin Country Name
        pl.when(is_jp_expr)
          .then(pl.col("Trip Destination Country Name").cast(pl.Utf8).str.strip_chars())
          .otherwise(pl.col("Trip Origin Country Name").cast(pl.Utf8).str.strip_chars())
          .alias("DIM_LOOKUP_KEY"),
          
        pl.col("Value").cast(pl.Utf8).str.replace_all(",", "").cast(pl.Float64, strict=False).fill_null(0.0).alias("Value_num")
    ])

    # 📌 Dimension.xlsx AA/AB열 기반 OD Region 매핑
    ...
    print(f"🎉 성공! 캐시 파켓 파일('{output_parquet_path}')이 정상 생성되었습니다! ({len(df_aggregated):,}행)")
