...
if selected_group == "✈️ 3/4수송 대시보드":
    ...
    # -------------------------------------------------------------
    # 3. 🏷️ 대리점,RBD별 발매현황 탭 (📌 단일 아코디언 + 스위치 + #cccccc 적용)
    # -------------------------------------------------------------
    with tab_34_3:
        ...
        st.download_button(label="📥 필터링된 대리점/RBD Raw Data (CSV) 전체 다운로드", data=csv_ag, file_name=f"Agency_RBD_Raw_Data_{datetime.date.today().strftime('%Y%m%d')}.csv", mime="text/csv")
    ...
    with tab_34_4:
        ...
        with st.expander("🔍 **단체실적 검색 피벗 슬라이서 필터 설정**", expanded=True):
            ...
            g_month_col = '출발월' if '출발월' in df_grp_raw.columns else '출발 월'
            all_g_months = sorted([str(x) for x in df_grp_raw[g_month_col].dropna().unique()]) if g_month_col in df_grp_raw.columns else []
            sel_g_month_str = render_slicer_box(gf_col2, "2. 출발월", all_g_months, "slicer_month_grp")
            raw_g_al = sorted([str(x) for x in df_grp_raw['Dominant Marketing Airline'].dropna().unique()])
            all_g_al = ['KE'] + [x for x in raw_g_al if x != 'KE']
        ...
        if g_month_col in df_grp_raw.columns and sel_g_month_str != ALL_OPTION: mask_grp &= (df_grp_raw[g_month_col].astype(str) == sel_g_month_str)
        ...
        if not df_grp_filtered.empty and 'Travel Agency Name' in df_grp_filtered.columns:
            ...
            al_sorted_list = ['KE'] + [str(x) for x in al_totals.index if x != 'KE']
...
# ==========================================
# GROUP 2: 🌐 6수송 대시보드 (종속 8번 필터 및 전체 YoY 연동 유지)
# ==========================================
elif selected_group == "🌐 6수송 대시보드":
    st.subheader("🌐 6수송 OD별 발매량, M/S 및 전년비(YoY) 분석 대시보드")
    
    if df_6th_raw is None:
        st.warning("👈 좌측 사이드바의 **[4. 6수송 데이터 (6TRF TEST.csv)]** 항목에 6수송 데이터 파일을 업로드해 주세요.")
        st.stop()
    ...
    def prepare_6th_freedom_dataset(df_in):
        ...
        actual_cols = {k: next((c for c in p if c in df_6_raw.columns), None) for k, p in col_map_6th.items()}
        ...
        val_col_6 = next((vc for vc in ['Value', 'VALUE', 'Pax', 'PAX', '실적', '발매량', 'Seats', 'Flights'] if vc in df_6_raw.columns), df_6_raw.columns[-1])
        ...
        if not is_cy_mask.any() and not is_py_mask.any(): is_cy_mask = pd.Series(True, index=df_6_raw.index)

        df_cy = df_6_raw[is_cy_mask].copy(); df_cy['Val_num'] = df_cy['Val_raw']; df_cy['Val_PY_num'] = 0.0
        df_py = df_6_raw[is_py_mask].copy(); df_py['Val_num'] = 0.0; df_py['Val_PY_num'] = df_py['Val_raw']

        df_out = pd.concat([df_cy, df_py], ignore_index=True) if not df_cy.empty or not df_py.empty else df_6_raw.copy()
        return optimize_df(df_out), actual_cols

    df_6, actual_cols = prepare_6th_freedom_dataset(df_6th_raw)

    al_col_6, od_col_6, month_col_6 = actual_cols['항공사'], actual_cols['OD ON/OFF'], actual_cols['TRIP MONTH']
    sorted_6th_airlines = ['KE'] + [x for x in df_6.groupby(al_col_6, observed=False)['Val_num'].sum().sort_values(ascending=False).index.astype(str).tolist() if x != 'KE']
    all_raw_m = sorted([str(x) for x in df_6[df_6['Val_num'] > 0][month_col_6].dropna().unique()]) if month_col_6 in df_6.columns else []

    act_dir_c, act_stop_c, act_onoff_c, act_reg_c, act_sub_c, act_ov_c = actual_cols['DIRECTION'], actual_cols['STOP OVER'], actual_cols['ON/OFF 여부'], actual_cols['4.OD RGN'], actual_cols['Sub-Route'], actual_cols['해외 APO']
    ...
    all_sub_6 = [str(x) for x in df_6.groupby(act_sub_c, observed=False)['Val_num'].sum().sort_values(ascending=False).index if pd.notnull(x)] if act_sub_c in df_6.columns else []
    all_ov_6 = sorted([str(x) for x in df_6[act_ov_c].dropna().unique()]) if act_ov_c in df_6.columns else []

    def filter_month_yoy(df_target, selected_m_val):
        if selected_m_val == ALL_OPTION or not month_col_6 or month_col_6 not in df_target.columns: return pd.Series(True, index=df_target.index)
        if '-' in str(selected_m_val): return df_target[month_col_6].astype(str).str.endswith(str(selected_m_val).split('-')[-1]) | (df_target[month_col_6].astype(str) == selected_m_val)
    ...
    with tab6_1:
        ...
        # 📌 1~7번 필터 결과에 연동되는 8번 종속 O&D 필터 옵션
        mask_base_1_to_7 = filter_month_yoy(df_6, sel_1_month)
        if act_reg_c in df_6.columns and sel_1_region != ALL_OPTION: mask_base_1_to_7 &= (df_6[act_reg_c].astype(str) == sel_1_region)
        if act_dir_c in df_6.columns and sel_1_dir != ALL_OPTION: mask_base_1_to_7 &= (df_6[act_dir_c].astype(str) == sel_1_dir)
        if act_stop_c in df_6.columns and sel_1_stop != ALL_OPTION: mask_base_1_to_7 &= (df_6[act_stop_c].astype(str) == sel_1_stop)
        if act_onoff_c in df_6.columns and sel_1_onoff != ALL_OPTION: mask_base_1_to_7 &= (df_6[act_onoff_c].astype(str) == sel_1_onoff)
        if act_sub_c in df_6.columns and sel_1_jp_route != ALL_OPTION: mask_base_1_to_7 &= (df_6[act_sub_c].astype(str) == sel_1_jp_route)
        if act_ov_c in df_6.columns and sel_1_ov_apo != ALL_OPTION: mask_base_1_to_7 &= (df_6[act_ov_c].astype(str) == sel_1_ov_apo)

        df_dep_for_od = df_6[mask_base_1_to_7]
        dependent_od_list = [str(x) for x in df_dep_for_od.groupby(od_col_6, observed=False)['Val_num'].sum().sort_values(ascending=False).index if pd.notnull(x)] if od_col_6 in df_dep_for_od.columns and not df_dep_for_od.empty else []
        sel_1_od_mkt = render_slicer_box(f1_col8, "8. Trip O&D Market", dependent_od_list, "slicer1_od_mkt")

        mask_tab1 = mask_base_1_to_7.copy()
        if od_col_6 in df_6.columns and sel_1_od_mkt != ALL_OPTION: mask_tab1 &= (df_6[od_col_6].astype(str) == sel_1_od_mkt)

        filtered_tab1 = df_6[mask_tab1].copy()

        if not filtered_tab1.empty and al_col_6 in filtered_tab1.columns:
            al_agg = filtered_tab1.groupby(al_col_6, observed=False)[['Val_num', 'Val_PY_num']].sum().reset_index().sort_values(by='Val_num', ascending=False)
            full_al_ranking = [str(x) for x in al_agg[al_col_6].tolist()]
            ke_rank = (full_al_ranking.index('KE') + 1) if 'KE' in full_al_ranking else "-"
            airline_rank_list = ['KE'] + [x for x in full_al_ranking if x != 'KE'][:10]
            ...
            for al_code in airline_rank_list:
                ...
                else: html_table += f'<th class="carrier-header" style="width:110px;"><div style="font-size:10px; opacity:0.85;">{full_al_ranking.index(al_code)+1}위</div>{al_code}</th>'
            html_table += '</tr></thead><tbody>'

            t_curr, t_prev = al_agg['Val_num'].sum(), al_agg['Val_PY_num'].sum()
            t_yoy_pct = ((t_curr - t_prev) / t_prev * 100) if t_prev > 0 else 0

            html_table += f'<tr class="row-title"><td>전체 발매</td><td><b>{t_curr:,.0f}</b></td>' + ''.join([f'<td><b>{al_agg[al_agg[al_col_6] == al]["Val_num"].sum():,.0f}</b></td>' for al in airline_rank_list]) + '</tr>'
            html_table += f'<tr><td style="color:#64748b; font-weight:600;">YOY</td><td>{"<span class=\"yoy-up\">▲ "+f"{t_yoy_pct:.0f}%</span>" if t_yoy_pct>=0 else "<span class=\"yoy-down\">▼ "+f"{abs(t_yoy_pct):.0f}%</span>"}</td>'
            for al in airline_rank_list:
                c_v, p_v = al_agg[al_agg[al_col_6] == al]['Val_num'].sum(), al_agg[al_agg[al_col_6] == al]['Val_PY_num'].sum()
                y_v = ((c_v - p_v) / p_v * 100) if p_v > 0 else 0
                html_table += f'<td>{(f"<span class=\"yoy-up\">▲ {y_v:.0f}%</span>" if y_v>=0 else f"<span class=\"yoy-down\">▼ {abs(y_v):.0f}%</span>") if p_v>0 else "-"}</td>'
            html_table += '</tr>'

            html_table += f'<tr class="row-title"><td>전체 M/S</td><td><b>100%</b></td>' + ''.join([f'<td><b>{(al_agg[al_agg[al_col_6] == al]["Val_num"].sum()/t_curr*100):.0f}%</b></td>' for al in airline_rank_list]) + '</tr>'
