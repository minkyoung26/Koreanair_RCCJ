...
# ==========================================
# GROUP 2: 🌐 6수송 대시보드 (요청 데이터셋 로직 완전 통합)
# ==========================================
elif selected_group == "🌐 6수송 대시보드":
    st.subheader("🌐 6수송 OD별 발매량, M/S 및 전년비(YoY) 분석 대시보드")
    
    if df_6th_raw is None:
        st.warning("👈 좌측 사이드바의 **[3. 6수송 데이터]** 항목에 6수송 데이터 파일(CSV, XLSX, ZIP)을 업로드해 주세요.")
        in_tab_6th = st.file_uploader("📂 또는 여기에서 6수송 데이터 파일(CSV, XLSX, ZIP)을 업로드하세요:", type=['csv', 'xlsx', 'zip', 'parquet'], key="in_tab_6th_uploader")
        if in_tab_6th:
            in_tab_6th.seek(0)
            if in_tab_6th.name.endswith('.parquet'):
                df_6th_raw = optimize_df(pd.read_parquet(in_tab_6th))
            else:
                df_6th_raw = optimize_df(pd.read_csv(in_tab_6th, low_memory=False))
        else:
            st.stop()
...
# ==========================================
    ...
    st.iframe(EXT_WEB_APP_URL, height=850)# app.py
import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
